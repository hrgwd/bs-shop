          ...
          <div class="table">
            <el-table :data="ordersData" strip>
              ...
              <el-table-column label="操作" align="center" width="180">
                <template v-slot="scope">
                  <!-- 待付款：可去付款、可取消 -->
                  <el-button size="mini" type="warning" v-if="scope.row.status === '待付款'" plain @click="pay(scope.row)">去付款</el-button>
                  <!-- 待发货：商家还没发货，仅展示状态，用户无操作 -->
                  <!-- 待收货：确认收货 -->
                  <el-button size="mini" type="primary" v-if="scope.row.status === '待收货'" plain @click="updateStatus(scope.row, '已完成')">确认收货</el-button>
                  <!-- 已完成：去评价 -->
                  <el-button size="mini" type="success" v-if="scope.row.status === '已完成'" plain @click="addComment(scope.row)">评价</el-button>
                  <!-- 已评价：追评 -->
                  <el-button size="mini" type="info" v-if="scope.row.status === '已评价'" plain @click="addComment(scope.row)">追评</el-button>
                  <!-- 只有待付款和已评价可以删除 -->
                  <el-button size="mini" type="danger" v-if="scope.row.status === '待付款' || scope.row.status === '已评价'" plain @click="del(scope.row.id)">删除</el-button>
                </template>
              </el-table-column>
            </el-table>
...
export default {
  ...
  methods: {
    ...
    pay(row) {
      // 只支付当前这一条订单
      window.location.href = `http://localhost:9090/alipay/pay?orderIds=${encodeURIComponent(row.orderId)}`
    }
  }
}
</script>
