  ...
  <div>
    ...
    <div class="table">
      <el-table :data="tableData" stripe  @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="description" label="商品详情" >
    ...
    <el-dialog title="商品详情" :visible.sync="editorVisible" width="50%">
...
<script>
import E from 'wangeditor'  // 导入富文本编辑器组件
  ...
  methods: {
    ...
    handleAdd() {   // 新增数据
      if ('审核通过' !== this.user.status) {
        this.$message.warning('您的店铺账号正在审核中，暂时无法进行操作')
        return
      }  //禁止非审核通过商家进行操作
   ...
   cancel() {
    ...
    save() {   // 保存按钮触发的逻辑  它会触发新增或者更新
      this.$refs.formRef.validate((valid) => {
        if (valid) {
          this.form.description = editor.txt.html()  // 获取富文本编辑器中的内容
          this.$request({
